...
// Copyright (c) 2013-2019 Adrian Kennard, Andrews & Arnold Limited, see LICENSE file (GPL)
//
...
int
read_unsigned (ui8 ** p, ui8 const *const e)
{
   int v = 0;
   while (*p < e && isdigit (**p))
      v = v * 10 + (*(*p)++ - '0');
...
{                               // process incoming audio, then run script
   if (debug)
            ...
            if (len > 12)
            {
               ...
               {                // DTMF/key
                  ...
                  if (erequest > request && (erequest[-1] == '*' || erequest[-1] == '#') && buf[12] <= 11)
                     done = "0123456789*#"[(int) buf[12]];      // Quit with key if end of playback is a * (wait) or # (exit at end)
               }
   ...
   if (!channels)
   {
      if (temp_fd >= 0)
         close (temp_fd);
      syslog (LOG_INFO, "%d Audio finished %u bytes%s", port, datalen, now > timeout ? " (timeout)" : "");
      return done;
   }

   syslog (LOG_INFO, "%d Audio finished %us%s", port, datalen / channels / 8000, now > timeout ? " (timeout)" : "");
   ...
   {                            // Run script
      ...
      {
         dup2 (open (outfilename, O_RDONLY), 0);        // stdin is the wav file
         unlink (outfilename);
         if (datalen)
         {                      // Recording
            if (!recscript)
      ...
      {                         // Copy some key headers
         ...
         if ((p = sip_find_header (rx, rxe, "To", "t", &e, NULL)))
         {
            ...
            if (rport >= 0)
            {
               ...
               sip_add_extra (&txp, txe, "tag", temp, NULL, ';', 0);
            }
         }
      ...
      {                         // It is an invite, check there is no tag on the To header, as that would make it a re-invite
         ...
         {                      // Looks like a new INVITE - allocate port and fork
          ...
          struct sockaddr_in6 raddr = { sin6_family:family };
            ...
            if (!p)
            {
               char done = audio_in (rport, a, rx, rxe, nonanswer);     // child
               ui8 *e,
